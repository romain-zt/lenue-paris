...
function openDraftPR(): TrackingPR {
  ...
  const bodyFile = path.join(ROOT, ".github/scripts/.decompose-pr-body.md");
  fs.writeFileSync(
    bodyFile,
    [
...
\`.cursor/rules/feature-area-workflow.mdc\` §1a, and \`.cursor/rules/00-siso.mdc\` "Standing waiver".
...
## Goal
Turn the ready PRD into runnable pipeline input by completing, **in priority-band order from \`docs/project.config.md\`**, the \`.cursor/\` decomposition chain for every v0 Feature Area, following \`.cursor/rules/feature-area-workflow.mdc\` and \`.cursor/commands/feature-area.md\`:
...
- Run the **real** \`.cursor/checkers/scope-readiness-checker.md\` checks. A genuine \`CLEAR\` verdict is what authorizes each promotion — do not self-assert readiness.
