 ...
 *
 * Run via:
 *   CURSOR_API_KEY=... tsx .github/scripts/validate-cursor-models.ts
  ...
  if (candidates.length === 0) {
    ...
    return {
      ...
      message:
        ...
        `      Known ids: ${knownIds}\n` +
        `      Run 'tsx .github/scripts/discover-cursor-models.ts' for the full table.`,
    };
  }
...
async function main(): Promise<void> {
  ...
  if (failed.length > 0) {
    ...
    console.error(
      "   Sync .github/scripts/cursor-models.config.ts → TIER_PARAMS using the discovery table.",
    );
    process.exit(1);
  }
