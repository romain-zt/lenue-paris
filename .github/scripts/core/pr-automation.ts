 ...
 * and enforces whatever .cursor/core/rules/ the cloned template ships — no hardcoded
...
- \`.cursor/core/rules/\` — the active governance rules this PR must satisfy. Do not deep-audit; just identify the load-bearing constraints.
...
1. **Governance boundaries** — respect the architecture baseline and any boundary/layering rules in \`.cursor/core/rules/\`. Do not let the diff violate them.
...
3. **Tests** — per \`.cursor/core/rules/30-test-strategy.mdc\`: behavior changes need tests; prefer contract/integration/unit over e2e. Flag missing tests.
