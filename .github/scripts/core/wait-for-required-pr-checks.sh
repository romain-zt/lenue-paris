#!/usr/bin/env bash
# Wait until the required GitHub check runs succeed on a PR head.
#
# REQUIRED_CHECKS defaults to "quality" (the CI job). Add "playwright" (E2E) or
# any other job name once those checks exist in the repo:
#   REQUIRED_CHECKS=quality,playwright
#
# Resilience: bounded by MAX_WAIT_SEC (default 1800s = 30 min) so a callsite can
# never hang indefinitely. WAIT_MODE=strict does a single pass (no polling).
set -euo pipefail

REPO="${REPO:?REPO is required}"
PR_SHA="${PR_SHA:-}"
PR_NUMBER="${PR_NUMBER:-}"
REQUIRED_CHECKS="${REQUIRED_CHECKS:-quality}"
