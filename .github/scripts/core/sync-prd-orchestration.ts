 ...
 * - docs/prd/PRD.md "# Flow Inventory" (rows whose v0 column starts with "Yes", in table order)
 ...
 *
 * Project-agnostic: the only project coupling is the PRD Flow Inventory table shape and
 * the docs/product/{feature-areas,scope-slices} layout used by the .cursor workflow.
 *
 * CLI:
 *   --print-flow-keys    Print normalized keys for current PRD Yes flows (maintainer helper)
 *   --strict             Exit 1 if any PRD Yes flow is missing from the manifest flows map
 */
...
function inferFeatureAreaFile(sliceRelPath: string, sliceContent: string): string {
  ...
  throw new Error(
    `Cannot infer feature area file for ${sliceRelPath} (fix Parent Feature Area link or add docs/product/feature-areas/${stem}.md)`,
  );
}
...
function pipelineHasSlice(pipeline: PipelineConfig, scopeSliceFile: string): boolean {
  return pipeline.steps.some(
    (s) =>
      ...
      s.workload.scopeSliceFile === scopeSliceFile,
  );
}
...
function main(): void {
  ...
  if (yesFlows.length === 0) {
    console.log("ℹ️  PRD has no '# Flow Inventory' table with v0=Yes rows yet — nothing to sync.");
    process.exit(0);
  }
