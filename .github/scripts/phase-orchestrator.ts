...
function sliceWorkloadMarkdown(stepRow: PipelineStepRow): string {
  ...
  const plan = w.planFile?.trim()
    ...
    : `- Implementation plan: if missing and the implementation phase requires one, produce it per \`.cursor/commands/\`; until approved, set this step \`blocked\` + NEED_HUMAN referencing the Slice path.\n`;
