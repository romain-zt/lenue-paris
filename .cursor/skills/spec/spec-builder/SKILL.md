...
Before executing any mode (except `check` which is mechanical), confirm that a Spec Context Brief has been produced by the Spec Lead (`.cursor/agents/spec/spec-lead.md`).
...
5. Invoke Spec Critic (`.cursor/agents/spec/spec-critic.md`) to review.
6. Present using the output format in `.cursor/commands/spec.md` Mode: propose.

## 5. Mode: scaffold

Materialize an **approved** Spec proposal into files. Governed by `.cursor/commands/spec.md` Mode: scaffold.
...
3. Else instantiate from `.cursor/templates/product/spec.template.md`.
...
Edit product + implementation sections of one Spec. Governed by `.cursor/commands/spec.md` Mode: refine.
