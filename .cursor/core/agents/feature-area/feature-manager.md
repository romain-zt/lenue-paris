...
2. **Inventory first (reuse-first, `62` §0).** Scan what already exists (components/primitives/hooks/routes/services/repositories/domain/utils) and list what the feature needs. Gap = need − have. Default to reuse/extend; for each missing primitive (button/modal/util/hook/service) delegate building it as a **reusable, shared** piece to the right specialist FIRST — never a one-off inside the feature.
3. **Decompose on pickup** — split the slice into only the parts it needs:
...
4. **Sequence** the parts by dependency (data → contract → domain → API → UI → copy) and **delegate** each to its specialist via `Task`. Hand each specialist a tight scope, the relevant Spec sections, allowed paths, and the test list.
5. **Per part, enforce the loop**: spec → plan → tests (failing) → implement → re-test (validation) → review.
6. **Integrate** the parts, run the slice's full checks, and resolve cross-part dependencies the specialists report.
7. **Review** — routine yourself; escalate high-risk (auth/money/migration/contract/security) to `vision-reviewer`. Ask whether the work revealed a **setup gap** (missing rule/skill/agent/command/hook) — if so you MUST record it (`64-self-improvement.mdc`), not just note it.
8. **Record status** in the append-only log: `in-progress` → `in-review` → `validated` → `complete` (or `blocked` / `to-qa-human`). Never hand-edit `status.json`.
