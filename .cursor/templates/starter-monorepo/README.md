...
(`.cursor/rules/40-architecture-baseline.mdc`). Fork it to start a project fast.
...
cp -R .cursor/templates/starter-monorepo my-project && cd my-project
...
- Integration tests run against this docker-compose stack (see `.cursor/rules/30-test-strategy.mdc`).
