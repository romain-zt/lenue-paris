...
- **Follow the baseline** (`.cursor/rules/40-architecture-baseline.mdc`): monorepo, Payload (i18n + S3), Postgres, MinIO local. Integration tests run against docker-compose.
- **Minimize e2e** (`.cursor/rules/30-test-strategy.mdc`): prefer unit + contract + integration.
