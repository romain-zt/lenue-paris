...
describe("storefrontCatalogue", () => {
  it("PUBLIC_DRESS_SLUGS has exactly three signature robes (AC-2)", () => {
    expect(PUBLIC_DRESS_SLUGS).toEqual(["robe-camille", "robe-louise", "robe-margot"]);
    expect(PUBLIC_DRESS_SLUGS).toHaveLength(3);
  });

  it("isPublicStorefrontSlug accepts all three signature slugs (AC-2)", () => {
  ...
  it("isPublicStorefrontSlug rejects non-signature dresses (AC-4)", () => {
    expect(isPublicStorefrontSlug("robe-heloise")).toBe(false);
  ...
  it("filterStorefrontProducts keeps all three signature dresses (AC-2)", () => {
